...
from optim.loss import loss_function,init_center,get_radius,EarlyStopping,randn_noise
...
def train(args,logger,data,model,path):
    ...
    logger.info(f'n-epochs:{args.n_epochs}, n-hidden:{args.n_hidden},n-layers:{args.n_layers},weight-decay:{args.weight_decay},noise_p:{args.noise_p},noise_d:{args.noise_d}')
    ...
    input_g=data['g']
    noise  = randn_noise(input_feat.size(0),args.n_hidden*2,args.noise_p,'rand',device)
    ...
    for epoch in range(args.n_epochs):
        ...
        auc,ap,f1,acc,precision,recall,val_loss,precision1,precision2,precision3 = fixed_graph_evaluate(args,checkpoints_path, model, data_center,data,radius,noise,data['val_mask'])
        #保存验证集AUC
    ...
    auc,ap,f1,acc,precision,recall,loss,precision1,precision2,precision3 = fixed_graph_evaluate(args,checkpoints_path,model, data_center,data,radius,noise,data['test_mask'])
    test_dur = 0
    #保存测试集AUC
