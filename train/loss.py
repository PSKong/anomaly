...
def loss_function(nu,data_center,outputs,radius=0,mask=None):
    ...
    ABS_LOSS =torch.abs((torch.mean(torch.abs( scoresA))-torch.mean(torch.abs(scoresB))))
    loss = radius ** 2 + (1 / nu) * torch.mean(torch.max(torch.zeros_like(scoresB), scoresB))+ABS_LOSS
...
def randn_noise(node,feat,noise_p,spread,device):
    if spread== 'rand':
        noise = torch.rand(node,feat,device=device)
    elif spread == 'randn':
        noise = torch.randn(node,feat,device=device)
    return noise_p*noise
