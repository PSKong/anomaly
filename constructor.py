...
def update(model, opt, sess, adj_norm, adj_label, features, placeholders, adj):
    ...
    attribute_reconstructions = sess.run(model.attribute_reconstructions, feed_dict=feed_dict)
    structure_reconstructions = sess.run(model.structure_reconstructions, feed_dict=feed_dict)
    # scipy.io.savemat('data/test/X_tf_subgraph_24.mat', {'X': emb})

    _, reconstruct_loss, reconstruction_errors = sess.run([opt.opt_op, opt.cost, opt.reconstruction_errors], feed_dict=feed_dict)


    return reconstruction_errors, reconstruct_loss, emb, attribute_reconstructions, structure_reconstructions